<%-- 
    Document   : dashboard
    Created on : May 19, 2026, 3:26:09 PM
    Author     : Kishor Mohan
--%>
            ...
            <div class="card shadow-lg border-0 rounded-4 p-4 p-md-5 text-center"
                ...
                <div class="card-body">
                    <img src="data:image/jpeg;base64,<%= user.getBase64Image()%>"
                         alt="Profile Picture" class="profile-img rounded-circle shadow-sm mb-4" /><h2 class="fw-bold text-dark mb-2">Welcome, <%= user.getFullname()%>!</h2>
                    <p class="text-muted fs-5 mb-4">Matric Number: <%=user.getMatricNo()%> <strong class="text-secondary"><div class="d-flex justify-content-center gap-3 flex-wrap">
                                <a href="UserServlet?action=logout" class="btn btn-dark px-4 py-2
                                   fw-semibold">Logout</a>
                                <a href="UserServlet?action=delete"
                                   onclick="return confirm('WARNING: Are you sure you want to delete
            your account permanently?');"
                                   class="btn btn-danger px-4 py-2 fw-semibold">Delete Account</a>
                                <a href="<%= request.getContextPath() %>/SubjectServlet?action=view"
   class="btn btn-success px-4 py-2 fw-semibold">My Subjects</a>
                            </div>
                </div>
            </div>
        </div>
